$(function () {
    ...
    const table = $('#data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: APP_URL + '/app-reference-type',
            type: 'GET',
        },
        columns: [
            {data: 'id', name: 'id'},
    ...
    $form.on('submit', function (e) {
        ...
        if ($form.parsley().isValid()) {
            ...
            axios
                .post(APP_URL + '/app-reference-type', formData)
                .then(function (response) {
                    if ($("#form-method").val() === 'add') {
                        $form[0].reset();
                    }
                    setTimeout(function () {
                        window.location.href = APP_URL + '/app-reference-type'
                    }, 1000);
    ...
    function deleteRecord(value_id) {
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/app-reference-type' + '/' + value_id,
    ...
    function changeStatus(value_id, status) {
        $.ajax({
            type: 'GET',
            url: APP_URL + '/referenceTypeChangeStatus/' + value_id + '/' + status,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
