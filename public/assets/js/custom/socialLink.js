$(function () {
    ...
    const table = $('#data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: APP_URL + '/social-link',
            type: 'GET',
        },
        columns: [
            {data: 'id', name: 'id'},
            {data: 'title', name: 'title'},
            {data: 'social_key', name: 'social_key'},
            {data: 'url', name: 'url'},
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success === true) {
                        ...
                        setTimeout(function () {
                            window.location.href = APP_URL + '/social-link'
                        }, 1000);
    ...
    function deleteRecord(value_id) {
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/social-link' + '/' + value_id,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
    ...
    function changeStatus(value_id, status) {
        $.ajax({
            type: 'GET',
            url: APP_URL + '/socialLinkChangeStatus/' + value_id + '/' + status,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
