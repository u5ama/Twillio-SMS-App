$(function () {
    ...
    $form.on('submit', function (e) {
        ...
        if ($form.parsley().isValid()) {
            ...
            axios
                .post(APP_URL + '/language-screen', formData)
                .then(function (response) {
                    ...
                    setTimeout(function () {
                        window.location.href = APP_URL + '/language-screen'
                    }, 1000);
    ...
    function deleteRecord(value_id) {
        ...
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/language-screen' + '/' + value_id,
    ...
    function changeStatus(value_id, status) {
        axios
            .get(APP_URL + '/language-screen/status' + '/' + value_id + '/' + status)
            .then(function (response) {
                table.draw()
                successToast(response.data.message, 'success');
            })
    ...
    const string = $('#view-string').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: APP_URL + '/viewScreenString/',
            type: 'GET',
            data: function (d) {
                d.id = $("#id").val()
            }
