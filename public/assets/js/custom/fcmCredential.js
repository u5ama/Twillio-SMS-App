$(function () {
    ...
    const table = $('#data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: APP_URL + '/fcm-credential',
            type: 'GET',
        },
        columns: [
            {data: 'id', name: 'id'},
            {data: 'project_name', name: 'project_name'},
            {data: 'server_key', name: 'server_key'},
    ...
    $form.on('submit', function (e) {
        ...
        if ($form.parsley().isValid()) {
            ...
            axios
                .post(APP_URL + '/fcm-credential', formData)
                .then(function (response) {
                    ...
                    setTimeout(function () {
                        window.location.href = APP_URL + '/fcm-credential'
                    }, 1000);
    ...
    function deleteRecord(value_id) {
        ...
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/fcm-credential' + '/' + value_id,
    ...
    function changeStatus(value_id, status) {
        axios
            .get(APP_URL + '/fcm-credential/status' + '/' + value_id + '/' + status)
            .then(function (response) {
                table.draw()
                successToast(response.data.message, 'success');
            })
