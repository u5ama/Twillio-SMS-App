$(function () {
    ...
    const table = $('#data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: APP_URL + '/app-reference-module',
            type: 'GET',
        },
        columns: [
            {data: 'id', name: 'id'},
    ...
    $form.on('submit', function (e) {
        ...
        if ($form.parsley().isValid()) {
            ...
            axios
                .post(APP_URL + '/app-reference-module', formData)
                .then(function (response) {
                    if ($("#form-method").val() === 'add') {
                        $form[0].reset();
                    }
                    setTimeout(function () {
                        window.location.href = APP_URL + '/app-reference-module'
                    }, 1000);
    ...
    function deleteRecord(value_id) {
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/app-reference-module' + '/' + value_id,
    ...
    function changeStatus(value_id, status) {
        $.ajax({
            type: 'GET',
            url: APP_URL + '/referenceModuleChangeStatus/' + value_id + '/' + status,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
